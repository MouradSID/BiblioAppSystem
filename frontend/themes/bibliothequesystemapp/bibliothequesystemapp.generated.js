...
window.Vaadin['_vaadintheme_bibliothequesystemapp_globalCss'] = window.Vaadin['_vaadintheme_bibliothequesystemapp_globalCss'] || [];
export const applyTheme = (target) => {
  
  const injectGlobal = (window.Vaadin['_vaadintheme_bibliothequesystemapp_globalCss'].length === 0) || (!window.Vaadin['_vaadintheme_bibliothequesystemapp_globalCss'].includes(target) && target !== document);
  if (injectGlobal) {
    injectGlobalCss(stylesCss.toString(), target);
    
    window.Vaadin['_vaadintheme_bibliothequesystemapp_globalCss'].push(target);
  }
  if (!document['_vaadintheme_bibliothequesystemapp_componentCss']) {
    registerStyles(
      'vaadin-button',
      css`
    ...
    document['_vaadintheme_bibliothequesystemapp_componentCss'] = true;
  }
